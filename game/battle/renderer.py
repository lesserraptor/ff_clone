...
from game.battle.layout import get_battle_layout
from game.sprites import get_sprite_atlas


class BattleRenderer(SceneRenderer):
    def __init__(self):
        self._prev_scale = 0
        self._texts = {}
    ...
    #  Main dispatch                                                      #
    # ------------------------------------------------------------------ #
    def draw(self, render_state, scale: float, width: int, height: int, **kwargs):
        state = kwargs.get("state", "")
        state_obj = kwargs.get("state_obj")
        flash_state = kwargs.get("flash_state")
        ...
        self.draw_enemies(render_state, width, height, scale)
        ...
        if state == "flash":
            ...
            self._draw_target_enemies(render_state, scale, width, height, state_obj)
        elif state.startswith("target_party"):
            self._draw_target_party(render_state, scale, width, height, state_obj)
        elif state in dispatch:
            dispatch[state](render_state, scale, width, height, state_obj)
        else:
            self._draw_message(render_state, scale, width, height, state_obj)

        # 4. Flash overlay (drawn on top of everything)
        if flash_state is not None or state == "flash":
            self._draw_flash(render_state, scale, width, height, flash_state)
    ...
    # ------------------------------------------------------------------ #
    def draw_enemies(self, render_state, w, h, scale):
        layout = get_battle_layout(scale)
        enemies = render_state.enemies
        if not enemies:
            return
        spacing = w / (len(enemies) + 1)
        cy = int(h * layout.enemy_cy_ratio)
        for i, enemy in enumerate(enemies):
            cx = spacing * (i + 1)
            sprite_id = enemy.name.lower().replace(" ", "_")
            if self._sprite_atlas.has_sprite(sprite_id):
                if enemy.alive:
                    self._sprite_atlas.draw(sprite_id, cx, cy, scale * layout.sprite_scale)
                elif enemy.dying_timer > 0:
                    self._draw_death_animation(sprite_id, cx, cy, scale * layout.sprite_scale, enemy.dying_timer, 0.5)
    ...
    def _draw_party_command(self, render_state, scale, w, h, state_obj):
        ...
        for mi, member in enumerate(render_state.party):
            ...
            if not member.alive:
                hp_str = "DEAD"
            self._text(f"pc_hp_{mi}", hp_str, scx + layout.char_spacing, scy,
                       hp_color, layout.font_size_sm, anchor_y="center")

        # --- Action box (overlapping both) ---
        ax = layout.enemy_box_x - bm
        aw = layout.action_box_w_scale * me
        ah = layout.action_box_h_scale * me
        ay = (layout.menu_h - ah) // 2
        # Clamp
        ax = max(0, ax)
        ...
        opt_y = ay + layout.text_padding
        opt_h = (ah - 2 * layout.text_padding) // 2
        for oi, opt in enumerate(opts):
            oy = opt_y + (len(opts) - 1 - oi) * opt_h + opt_h // 2
            color = COLORS["cursor"] if oi == sel else COLORS["text"]
            self._text(f"pc_opt_{oi}", opt, ax + layout.char_spacing + layout.cursor_offset, oy,
                       color, layout.font_size_md, anchor_y="center")
            if oi == sel:
                draw_cursor(ax + layout.cursor_offset, oy, scale, COLORS["cursor"])

    # ------------------------------------------------------------------ #
    #  Run outcome (Phase 2)                                              #
    # ------------------------------------------------------------------ #
    def _draw_run_outcome(self, render_state, scale, w, h, state_obj, message_log=None):
        self._draw_full_message_box(scale, w, h, state_obj, message_log)

    # ------------------------------------------------------------------ #
    #  Message (battle events during execution)                           #
    # ------------------------------------------------------------------ #
    def _draw_message(self, render_state, scale, w, h, state_obj, message_log=None):
        self._draw_full_message_box(scale, w, h, state_obj, message_log)
    ...
    # ------------------------------------------------------------------ #
    def _draw_char_command(self, render_state, scale, w, h, state_obj):
        layout = get_battle_layout(scale)
        me = layout.me
        bm = layout.border_px
        mh = layout.menu_h
        member_idx = state_obj.member_idx if state_obj else 0
        member = render_state.party[member_idx] if member_idx < len(render_state.party) else None
        ...
        if member:
            sprite_id = member.name.lower()
            scx = bm + layout.char_sprite_offset
            scy = bm + (mh - 2 * bm) // 2
            if member.alive:
                if not self._sprite_atlas.draw(sprite_id, scx, scy, scale * layout.char_sprite_scale):
                    sz = layout.line_h_md
                    arcade.draw_lrbt_rectangle_filled(scx - sz, scx + sz,
                                                       scy - sz, scy + sz,
                                                        (48, 48, 48))
            ...
            self._text(f"cc_name", member.name[:7], scx + layout.hp_bar_offset, scy + layout.char_spacing2,
                       nm_color, layout.font_size_md, anchor_y="center")
            # HP
            if member.alive:
                hp_str = f"HP {member.hp}/{member.hp_max}"
                self._text(f"cc_hp", hp_str, scx + layout.hp_bar_offset, scy - 2 * me,
                           COLORS["text"], layout.font_size_sm, anchor_y="center")
                # MP
                if member.mp_max > 0:
                    mp_str = f"MP {member.mp}/{member.mp_max}"
                    self._text(f"cc_mp", mp_str, scx + layout.hp_bar_offset, scy - layout.char_spacing2,
                               (120, 120, 120), layout.font_size_sm, anchor_y="center")
            else:
                self._text(f"cc_dead", "DEAD", scx + layout.hp_bar_offset, scy,
                           (120, 120, 120), layout.font_size_md, anchor_y="center")

        # --- Options box (right 3/5) ---
        ox = layout.enemy_box_x
        ow = layout.enemy_box_w
    ...
    # ------------------------------------------------------------------ #
    def _draw_spell_select(self, render_state, scale, w, h, state_obj):
        layout = get_battle_layout(scale)
        me = layout.me
        bm = layout.border_px
        mh = layout.menu_h
                ...
                if not self._sprite_atlas.draw(sprite_id, scx, scy, scale * layout.char_sprite_scale):
                    ...
                    arcade.draw_lrbt_rectangle_filled(scx - sz, scx + sz,
                                                       scy - sz, scy + sz,
                                                        (48, 48, 48))
            nm_color = COLORS["text"] if member.alive else (120, 120, 120)
            self._text(f"ss_name", member.name[:7], scx + layout.line_h_md, scy + layout.char_spacing2,
                       nm_color, layout.font_size_md, anchor_y="center")
            hp_str = f"HP {member.hp}/{member.hp_max}" if member.alive else "DEAD"
            self._text(f"ss_hp", hp_str, scx + layout.line_h_md, scy - 2 * me,
                       nm_color, layout.font_size_sm, anchor_y="center")
        ...
        for si, spell_id in enumerate(spells):
            spell = render_state.spells.get(spell_id, {})
            ...
            self._text(f"ss_spell_{si}", f"{name}  MP{mp_cost}", ox + 3 * bm, y,
                       color, layout.font_size_sm, anchor_y="center")
            if si == sel:
    ...
    # ------------------------------------------------------------------ #
    def _draw_target_enemies(self, render_state, scale, w, h, state_obj):
        layout = get_battle_layout(scale)
        me = layout.me
        bm = layout.border_px
        mh = layout.menu_h
        tbox_w = layout.target_box_w
        draw_window(0, 0, tbox_w, mh, scale,
                    fill_color=COLORS["box_fill"], border_color=COLORS["box_border"])
        sel = state_obj.selection if state_obj else 0
        targets = render_state.living_enemies
        if not targets:
            self._text("te_none", "No targets", bm, mh // 2,
                       (120, 120, 120), layout.font_size_md, anchor_y="center")
            return
        line_h = layout.line_h_md
        start_y = mh - bm - line_h
        for ti, target in enumerate(targets):
            y = start_y - ti * line_h
            color = COLORS["cursor"] if ti == sel else COLORS["text"]
            self._text(f"te_{ti}", target.name, tbox_w // 2, y,
                       color, layout.font_size_md, anchor_x="center", anchor_y="center")
            if ti == sel:
    ...
    # ------------------------------------------------------------------ #
    def _draw_target_party(self, render_state, scale, w, h, state_obj):
        layout = get_battle_layout(scale)
        me = layout.me
        bm = layout.border_px
        mh = layout.menu_h
        ...
        draw_window(0, 0, tbox_w, mh, scale,
                    fill_color=COLORS["box_fill"], border_color=COLORS["box_border"])
        sel = state_obj.selection if state_obj else 0
        targets = render_state.living_party
        if not targets:
    ...
    #  Item select                                                        #
    # ------------------------------------------------------------------ #
    def _draw_item_select(self, render_state, scale, w, h, state_obj):
        layout = get_battle_layout(scale)
        me = layout.me
        bm = layout.border_px
        mh = layout.menu_h
        ...
        for ii, item in enumerate(items):
            y = start_y - ii * line_h
            item_def = render_state.item_data.get(item["id"], {})
    ...
    # ------------------------------------------------------------------ #
    def _draw_execute(self, render_state, scale, w, h, state_obj):
        ...
        self._text("exec_title", "BATTLE!", w // 2, mh // 2,
                   (24, 24, 24), layout.font_size_lg,
                   anchor_x="center", anchor_y="center")

    # ------------------------------------------------------------------ #
    #  Victory                                                            #
    # ------------------------------------------------------------------ #
    def _draw_victory(self, render_state, scale, w, h, state_obj, message_log=None):
        layout = get_battle_layout(scale)
        me = layout.me
        mh = layout.menu_h
        draw_window(0, 0, w, mh, scale,
                    fill_color=COLORS["box_fill"], border_color=COLORS["box_border"])
        msg = state_obj.get_message(render_state) if state_obj else "Victory!"
        lines = msg.split("\n")
        line_h = layout.line_h_sm
        ...
        for li, line in enumerate(lines):
            ...
            self._text(f"vic_{li}", line, layout.text_padding, start_y - li * line_h,
                       color, layout.font_size_md, anchor_y="center")

    # ------------------------------------------------------------------ #
    #  Defeat                                                             #
    # ------------------------------------------------------------------ #
    def _draw_defeat(self, render_state, scale, w, h, state_obj, message_log=None):
        layout = get_battle_layout(scale)
        mh = layout.menu_h
        draw_window(0, 0, w, mh, scale,
                    fill_color=COLORS["box_fill"], border_color=COLORS["box_border"])
        msg = state_obj.get_message(render_state) if state_obj else "Defeat..."
        self._text("defeat_msg", msg, w // 2, mh // 2,
                   (48, 48, 48), layout.font_size_lg,
                   anchor_x="center", anchor_y="center")

    # ------------------------------------------------------------------ #
    #  Flash highlight                                                    #
    # ------------------------------------------------------------------ #
    def _draw_flash(self, render_state, scale, w, h, flash_state=None):
        ...
        if action.action_type in (ActionType.PARTY_ATTACK, ActionType.PARTY_MAGIC, ActionType.USE_ITEM):
            for mi, member in enumerate(render_state.party):
                if member.name == action.actor.name:
                    scx = layout.font_size_lg + layout.flash_sprite_sz
                    scy = (len(render_state.party) - 1 - mi) * layout.row_h + layout.row_h // 2
                    sz = layout.flash_sprite_sz
                    arcade.draw_lrbt_rectangle_outline(scx - sz - layout.flash_outline_sz,
                                                       scx + sz + layout.flash_outline_sz,
                                                       scy - sz - layout.flash_outline_sz,
                                                       scy + sz + layout.flash_outline_sz,
                                                        (24, 24, 24), layout.flash_outline_sz)
                    break
        elif action.action_type == ActionType.ENEMY_ATTACK:
            for ei, enemy in enumerate(render_state.enemies):
                if enemy.name == action.actor.name:
                    enemies = render_state.enemies
                    spacing = w / (len(enemies) + 1)
                    cx = spacing * (ei + 1)
                    cy = int(h * layout.enemy_cy_ratio)
                    sz = layout.enemy_flash_sz
                    arcade.draw_lrbt_rectangle_outline(cx - sz, cx + sz,
                                                       cy - sz, cy + sz,
                                                        (48, 48, 48), layout.flash_outline_sz)
                    break
